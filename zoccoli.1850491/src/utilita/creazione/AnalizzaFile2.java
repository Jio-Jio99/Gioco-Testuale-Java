 ...
 *
 */
public abstract class AnalizzaFile2 implements Observable{
	/**
	 ...
	 */
	public static final Map<String, CreationFunction> dizionario_funzioni = Map.of(	LINKS, AnalizzaFile2::creaLink,
																					CHARACTERS, AnalizzaFile2::creaPersonaggio,
																					OBJECTS, AnalizzaFile2::creaOggetto);																	
	...
	private static void controllo(Set<Stanza> stanze) throws ErroreCaricamentoException{
		...
		List<Entita> lista = stanze.stream()
									.map(AnalizzaFile2::supportoControllo)
									.flatMap(Set::stream)
									.collect(Collectors.toList());
